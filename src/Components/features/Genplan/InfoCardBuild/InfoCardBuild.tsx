...
import {houseTotalType, houseTotalsResponse} from "@/src/api/types/typesGenplan";

interface FloorTotalsData {
    floor: string | number;
    countPlaces: number;
    totals?: houseTotalType[];
}

type InfoCardData = houseTotalsResponse | FloorTotalsData;

interface IInfoCardBuildProps {
    data: InfoCardData | null | undefined | false;
            ...
            {data &&
				<div
					style={{
					 ...
					 className={clsx(
                         style.infoCardBuild,
                         mode === 'corps' && code && style[`code${code}`],
                         style.floors,
                         className
                     )}
				>
                    {mode === 'corps' && 'num' in data && <h3 className={style.infoCardBuild__title}>Корпус {data.num}</h3>}
                    {mode === 'floors' && 'floor' in data && <h3 className={style.infoCardBuild__title}>{data.floor} Этаж</h3>}
